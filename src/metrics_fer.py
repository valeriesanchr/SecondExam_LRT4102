...
class Recolector:
    ...
    def hacer_pdf(self):
        total = sum(sum(p) for p in self.tiempos)
        total_items = sum(len(p) for p in self.tiempos)
        prom_item = round(total / total_items, 2)
        prom_prueba = [round(sum(p), 2) for p in self.tiempos]
        ...
        for i, prueba in enumerate(self.tiempos, 1):
            ...
            pdf.set_font("Arial", size=12)
            for j, t in enumerate(prueba, 1):
                pdf.cell(200, 10, txt=f"  Obj {j}: {t} s", ln=True)
